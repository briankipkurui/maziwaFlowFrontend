...
export const useCooperativesQuery = (
    params: Ref<CooperativeListParams>,
) => {
    return useQuery({
        queryKey: computed(() =>
            cooperativeQueryKeys.list(params.value),
        ),
        queryFn: () => cooperativeService.list(params.value),
    });
};

export const useCooperativeQuery = (
    id: Ref<string>,
) => {
    return useQuery({
        queryKey: computed(() =>
            cooperativeQueryKeys.detail(id.value),
        ),
